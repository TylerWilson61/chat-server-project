...
#        set the dimensions to the window and the title
        ...
        # self.layout.addRow(self.errormsg_display)
    ...
    def client_w(self):
        #opens up our client window
        self.cw = client(self.socket,self.user_input.text())
        self.cw.client_window()
        msg_thread = threading.Thread(target = self.cw.run_chat, args = (), daemon = True)
        msg_thread.start()
    ...
    def open_client(self):
        ...
        #sends our login information to server
        ...
        #displays an error msg
            ...
            if len(self.login_msg) == 0:
                self.login_msg = response["errormsg"]
                login_btn_response = QLabel(self.login_msg)
